...
# 从 train_and_tsne.py 导入相同的模型结构
from train_and_tsne import SimCLRModel

# -------------------------------------------------------------
# 1. 线性探针评估 (Linear Probe)
# -------------------------------------------------------------
def evaluate_linear_probe(device, model, train_loader, test_loader, epochs=10):
    ...
    print(f"👉 线性评估最终 Test Accuracy: {test_acc*100:.2f}%")
    return test_acc
...
# -------------------------------------------------------------
@torch.no_grad()
def evaluate_full_knn(device, model, train_loader, test_loader, k=20):
    print("\n=== 开始进行全量测试集 KNN 评估 ===")
    model.eval()
    
    # 提取训练集全部 60,000 个样本特征
    print("正在提取训练集特征...")
    train_features = []
    train_labels = []
    ...
    # 提取测试集全部 10,000 个样本特征
    print("正在提取测试集特征...")
    test_features = []
    test_labels = []
    for x, y in tqdm(test_loader, desc="Test Features"):
        x = x.to(device)
        h = model.encoder(x)
        h = h.view(h.shape[0], -1)
        h = torch.nn.functional.normalize(h, dim=1)
...
def main():
    ...
    # 加载模型
    model = SimCLRModel(out_dim=128)
    ckpt_path = "simclr_fashionmnist.pth"
    if not os.path.exists(ckpt_path):
        print(f"错误: 找不到权重文件 '{ckpt_path}'！请先运行训练脚本 train_and_tsne.py")
        return
    ...
    # 2. 评估线性分类器（Linear Probe）准确率
    # 线性评估时重新 shuffle 训练数据
    train_loader_shuffle = DataLoader(train_dataset, batch_size=256, shuffle=True, num_workers=0)
    evaluate_linear_probe(device, model, train_loader_shuffle, test_loader, epochs=10)
