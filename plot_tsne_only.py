...
def plot_tsne(features_h, true_labels=None, mode="kmeans", perplexity=50, save_path="tsne_result.png"):
    ...
    参数:
    - features_h: 提取的 512 维特征
    - true_labels: 真实标签，仅在评估 NMI 分数或使用 supervised 模式时使用
    - mode: 
        1. "kmeans": 完全无监督。使用 K-Means 自动对特征聚为 10 类，并以聚类 ID 着色。
        2. "uncolored": 完全单色。只绘制高维特征点在低维空间中的天然物理分布（聚簇形状）。
        3. "supervised": 用真实标签着色（仅供内部开发对比使用）。
    ...
    if mode == "kmeans":
        ...
        # 计算无监督聚类质量的客观指标：NMI (归一化互信息)
        # NMI 越接近 1.0，说明无监督聚类和真实类别的重合度越高
